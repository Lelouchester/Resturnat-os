        ...
        {moreOpen && (
          <div className="md:hidden fixed inset-0 z-40" onClick={() => setMoreOpen(false)}>
            ...
            >
              ...
              <MobileAccountRow onSignOut={() => setMoreOpen(false)} />
            </div>
          </div>
        )}
      </div>
...
function MobileAccountRow({ onSignOut }: { onSignOut: () => void }) {
  const staff = useAuthStore((s) => s.staff)
  const signOut = useAuthStore((s) => s.signOut)
  if (!staff) return null

  return (
    <div className="mt-3 pt-3 border-t border-ink/5 flex items-center justify-between px-1">
      ...
      <button
        ...
        className="shrink-0 flex items-center gap-1.5 text-xs font-semibold rounded-full border border-ink/10 px-3 py-2 text-ink/60 hover:bg-ink/5"
      >
