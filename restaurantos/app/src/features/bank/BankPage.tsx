...
import { useBankLedgerStore } from './bankLedgerStore'
...
export function BankPage() {
  ...
  // Running balance reads every entry ever made (not just the filtered
  // range) in date order, so the balance shown next to a row in the
  // filtered view is still the true real-world balance as of that day —
  // not just a sum of whatever happens to be in the current date filter.
  ...
  return (
    <div className="p-4 md:p-6 max-w-3xl mx-auto">
      ...
      <p className="text-xs text-ink/40 mb-5">
        The real bank balance, kept by hand — separate from the app's own internal Bank account, which only tracks money moved through Accounts &amp; transfers. Every entry here is permanent once saved — a mistake gets corrected with a new entry, not by editing the old one, so this stays a trustworthy record of who entered what and when.
      </p>
          ...
          {[...filtered].reverse().map((e) => (
            <Card key={e.id} className="p-3.5">
              <div className="flex items-center justify-between mb-0.5">
                <span className="text-sm font-semibold">{e.remark}</span>
                <span className={`font-ticket font-bold text-sm ${e.amount >= 0 ? 'text-status-available' : 'text-status-cleaning'}`}>
                  {e.amount >= 0 ? '+' : ''}Rs. {e.amount.toLocaleString()}
                </span>
              </div>
              <div className="flex items-center justify-between text-xs text-ink/40">
                <span>
                  {new Date(e.entryDate).toLocaleDateString()} · {e.createdByName}
                </span>
                <span>Balance: Rs. {e.runningBalance.toLocaleString()}</span>
              </div>
            </Card>
          ))}
        </div>
