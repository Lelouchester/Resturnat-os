...
// Demo "sales so far this shift" — once billing writes to `payments`, this
// becomes a real query summed by payment method for the open shift's window.
const DEMO_SALES: MethodBalances = { cash: 8420, esewa: 4640, fonepay: 2740 }
const DEMO_ORDERS_SERVED = 14
...
export function ShiftsPage() {
  ...
  // Prefilled from yesterday's closing count — today's closing IS tomorrow's opening.
  ...
  // `lastClosing` arrives asynchronously from Supabase — the useState above
  // only ran once at mount, before that data existed, so sync it in here too.
  useEffect(() => {
    if (lastClosing) setOpening(lastClosing)
  }, [lastClosing])

  const elapsed = useElapsedTime(shift?.openedAt)
  const salesByMethod = DEMO_SALES
  const totalSales = paymentMethods.reduce((s, m) => s + (salesByMethod[m.key] || 0), 0)

  function expectedFor(key: string) {
    return (shift?.opening[key] ?? 0) + (salesByMethod[key] || 0)
  }
  ...
  function handleEndShift() {
    ...
    setOpening(closingBalances) // pre-fill tomorrow's start with today's close
  ...
  if (shiftLoading) {
    return (
      <div className="p-4 md:p-6 max-w-2xl mx-auto">
        <div className="mb-4">
          <h1 className="font-ticket text-xl font-bold">Shift</h1>
          <p className="text-sm text-ink/50">Start of day, end of day, and account reconciliation</p>
        </div>
    ...
    <div className="p-4 md:p-6 max-w-2xl mx-auto">
      ...
      {!shift ? (
        ...
        <>
          ...
          <Card className="p-5 mb-4">
            <div className="font-ticket text-xs font-bold uppercase tracking-wider text-ink/40 mb-3">Sales so far</div>
            {paymentMethods.map((m) => (
              <Row key={m.key} label={m.label} value={salesByMethod[m.key] || 0} />
            ))}
            <div className="border-t border-ink/10 mt-2 pt-2 flex justify-between items-baseline">
              <span className="text-sm font-semibold">Total</span>
              <span className="font-ticket text-lg font-bold">Rs. {totalSales}</span>
            </div>
            <div className="text-xs text-ink/40 mt-1">{DEMO_ORDERS_SERVED} orders served</div>
          </Card>

          {!closing ? (
            <Button variant="danger" className="w-full flex items-center justify-center gap-2" onClick={() => setClosingState(true)}>
