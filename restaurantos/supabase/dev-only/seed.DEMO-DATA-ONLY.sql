-- DEMO DATA ONLY — fake suppliers, fake staff, fake balances. Never run this
-- on a cafe with real data. It refuses to run if any orders already exist.
do $$
declare
  v_orders bigint;
begin
  if to_regclass('public.orders') is not null then
    ...
    if v_orders > 0 then
      raise exception 'STOPPED: this database already has orders — the demo seed was not run.';
    end if;
  end if;
end $$;
